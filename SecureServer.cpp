...
SecureServer::SecureServer(int portNumber)
{
    cout << "**** Secure Chat Server ****" << endl;
...
void SecureServer::startListener()
{
    ...
    m_srvAddr.sin_port = htons(serverListeningPort);
...
void SecureServer::eventHandler()
{
    char buffer[1024];
    SSL_library_init();
...
void SecureServer::Servlet(SSL *ssl) /* Serve the connection -- threadable */
{
    char buf[1024];
    char reply[1024];
    int sd, bytes;
...
void SecureServer::LoadCertificates(SSL_CTX *ctx, const char *CertFile, const char *KeyFile)
{
